"use strict";
// Exercise 1
const btnEvenOdd = document.querySelector("#btnEvenOdd");
btnEvenOdd.addEventListener("click", (e) => {
    e.preventDefault();
    evenOdd(10);
});
function evenOdd(e = 0) {
    if (e % 2 == 0) {
...
function weekend() {
    const day = prompt("Ingresa un día de la semana (por ejemplo, lunes(1), martes(2), miercoles (3), etc.):");
    if (day === null) {
        alert("Invalid");
        return;
    }
    ;
    const dayUser = parseInt(day);
    let mensaje;
    switch (dayUser) {
        case 1:
...
;
// Exercise 5
const btnNacionality = document.getElementById("btnNacionality");
btnNacionality.addEventListener("click", (e) => {
    e.preventDefault();
    nacionality();
});
function nacionality() {
    const country = prompt("Enter your nacionality");
    if (country == null) {
